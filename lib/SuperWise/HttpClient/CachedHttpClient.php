...
use SuperWise\HttpClient\Cache\CacheInterface;
use SuperWise\HttpClient\Cache\FilesystemCache;

/**
 * Performs requests on SuperWise API using If-Modified-Since headers.
...
class CachedHttpClient extends HttpClient
{
    ...
    {
        if (null === $this->cache) {
            $this->cache = new FilesystemCache($this->options['cache_dir'] ?: sys_get_temp_dir().DIRECTORY_SEPARATOR.'php-sirwise-api-cache');
        }
